...
class ThermalServer:
    def __init__(self, push_stream=None, server=None, model=None) -> None:
        ...
        self.color = [(0,255,0), (0,255,0), (0,0,255), (0,0,255), (0,255,0), (255,0,0), (255,0,0), (255,0,0)]
        self.status = "normal"
        self.isWork = True
    ...
    def receive_data(self):
        ...
        while True:
            ...
            if new_data is None:
                ...
                timebuffer.append(norm_data)
                if len(timebuffer) > 11:
                    timebuffer.pop(0)
                if len(timebuffer) < 11:
                    continue
                ...
                # merged_pic = cv2.merge((timebuffer[0], timebuffer[5], timebuffer[10]))
                merged_pic = [timebuffer[0],timebuffer[2],timebuffer[4],timebuffer[6],timebuffer[8]]
                merged_pic = np.transpose(merged_pic, axes=(1,2,0))
                if self.isWork:
                    ret_boxs = yolomodel.inference(merged_pic, conf_thres=0.2, iou_thres=0.4)
                    # ret_boxs = []
                else:
                    ret_boxs = []
                ...
                for pred_box in ret_boxs:
                    ...
                    if uid in name_dic.keys():
                        ...
                        name = fake.name()
                        name_dic[uid] = name
                    cv2.putText(rendered_pic, str(pred_box[4]), (box[0], box[1]-5),cv2.FONT_HERSHEY_SIMPLEX, 0.9, self.color[int(pred_box[4])%3], 2)
...
if __name__ == '__main__':
    onnx_model_path = "Models/onnx/c10d30.onnx"
