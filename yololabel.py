...
def read_npy():
    ...
    data_path = './data/classified_data/003/'
    ...
    while True:
        ...
        if new_data is None:
            ...
            for pred_box in ret_boxs:
                ...
                cv2.rectangle(rendered_pic, (box[0], box[1]), (box[2], box[3]), color[pred_box[4]], thickness=3)
                if pred_box[4] == 1:
                    fall_flag = True
...
def label_jpg(onnx_model_path, image_path, label_path):
    onnx_model_path = "../Models/onnx/toilet3.onnx"
    yolomodel = YOLOModel(onnx_model_path)
    label_dic = {"001":-1, "002":0, "003":-1, "004":1, "005":-1, "006":2,"007":2, "008":1}
    color = [(0,255,0), (0,0,255), (255,0,0)]
    ...
    for label_dir in os.listdir(image_path):
        ...
        if label >= 0:
            ...
            for filename in os.listdir(os.path.join(image_path, label_dir)):
                ...
                cv2.rectangle(image, (box[0], box[1]), (box[2], box[3]), color[pred_box[4]], thickness=3)
...
def make_dataset(data_path, dataset_path):
    ...
    def split_data(image_path, label_path):
        ...
        for i in data_num_list:
            ...
            src_image = os.path.join(image_path, record_name+'.jpg')
            src_label = os.path.join(label_path, record_name+'.txt')

            if i in train:
                dst_image = os.path.join(train_image_path, record_name+'.jpg')
                dst_label = os.path.join(train_label_path, record_name+'.txt')
            elif i in val:
                dst_image = os.path.join(val_image_path, record_name+'.jpg')
                dst_label = os.path.join(val_label_path, record_name+'.txt')
            elif i in test:
                dst_image = os.path.join(test_image_path, record_name+'.jpg')
